...
public class TestS3ADeleteOnExit extends AbstractS3AMockTest {
  ...
  public void testDeleteOnExit() throws Exception {
    ...
    when(testS3.getObjectMetadata(argThat(new RequestMatches(BUCKET, key))))
            .thenReturn(meta);
  ...
  /**
   * Mockito 1.x argument matcher for key and bucket.
   */
  private static final class RequestMatches extends ArgumentMatcher<GetObjectMetadataRequest> {
    private final String bucket, key;

    private RequestMatches(final String bucket, final String key) {
      this.bucket = bucket;
      this.key = key;
    }
    ...
    public boolean matches(final Object argument) {
      GetObjectMetadataRequest request = (GetObjectMetadataRequest) argument;
      return request != null
