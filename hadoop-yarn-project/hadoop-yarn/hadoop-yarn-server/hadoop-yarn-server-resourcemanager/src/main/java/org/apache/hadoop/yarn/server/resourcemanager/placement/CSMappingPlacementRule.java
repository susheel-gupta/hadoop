...
public class CSMappingPlacementRule extends PlacementRule {
  private static final Log LOG =
      LogFactory.getLog(CSMappingPlacementRule.class);
  ...
  public boolean initialize(ResourceScheduler scheduler) throws IOException {
    ...
    LOG.info("Initialized queue mappings, can override user specified " +
        "queues: " + overrideWithQueueMappings + "  number of rules: " +
        mappingRules.size() + " mapping rules: " + mappingRules);
  ...
  public ApplicationPlacementContext getPlacementForApp(
    ...
    LOG.debug("Looking placement for app '" + asc.getApplicationName() +
        "' originally submitted to queue '" + appQueue +
        "', with override enabled '" + overrideWithQueueMappings + "'");
    if (appQueue != null &&
        !appQueue.equals(YarnConfiguration.DEFAULT_QUEUE_NAME) &&
        !overrideWithQueueMappings &&
        !recovery) {
      LOG.info("Have no jurisdiction over application submission '" +
          asc.getApplicationName() + "', moving to next PlacementRule engine");
      return null;
    }
    ...
    for (MappingRule rule : mappingRules) {
      ...
      case PLACE:
        ...
        break;
      case REJECT:
        LOG.info("Rejecting application '" + asc.getApplicationName() +
            "', reason: Mapping rule '{}' fallback action is set to REJECT.");
    ...
    if (ret == null) {
      ...
      LOG.info("No matching rule found for application '" +
          asc.getApplicationName() + "', moving to next PlacementRule engine");
    }
  ...
  private ApplicationPlacementContext placeToQueue(
    ...
    LOG.debug("Application '" + asc.getApplicationName() +
        "' have been placed to queue '" + result.getNormalizedQueue() +
        "' by rule " + rule);
  ...
  private ApplicationPlacementContext placeToDefault(
    ...
    try {
      ...
      LOG.debug("Application '" + asc.getApplicationName() +
              "' have been placed to queue '" + queueName + "' by the " +
              "fallback option of rule " + rule);
      return createPlacementContext(queueName);
    } catch (YarnException e) {
      LOG.error("Rejecting application due to a failed fallback" +
          " action '" + asc.getApplicationName() + "'" + ", reason: " +
          e.getMessage());
      //We intentionally omit the details, we don't want any server side
      //config information to leak to the client side
